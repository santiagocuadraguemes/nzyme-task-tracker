...
- **Never delete `Priority = [DETAILS INSIDE]` rows in the Team Task Tracker.** Those are architecture/hierarchy nodes, not tasks. Deleting them breaks the classifier's parent-assignment context. The marker is also the single source of truth that splits the classifier (sees architecture only) from the deduper (sees tasks only).
