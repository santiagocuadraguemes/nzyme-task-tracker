...
- **Never edit member-DB option lists for `Work area`, `Detail`, or `External Org` directly.** All three flow from canonicals: `Work area` from Hierarchy DB Tier 0 → `hierarchy_rows`; `Detail` from the `Detail Options` Settings DB → `detail_rows`; `External Org` from `public."ReportingNz_deals"` (Supabase, filtered by stage). The daily `hierarchy_sync` cron PATCHes each member DB; manual edits are overwritten on the next run.
