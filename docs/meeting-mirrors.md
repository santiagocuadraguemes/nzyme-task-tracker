...
Notion `POST /v1/pages` with `template: {type: "template_id", template_id: <source_page_id>}` clones the source — including the AI-managed `meeting_notes` block (transcript, AI Summary, attendees, notes). Verified empirically in `scripts/replicate_meeting.py`. Properties not declared on the target DB schema are silently dropped, so the pipeline-control columns (`Processed`, `Processing`, `Template Injected`, `Task - Relation`, `LP Emails`) simply don't carry over — no need to filter them out in code.
