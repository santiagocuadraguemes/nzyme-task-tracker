...
`External Org` is **not** in this table anymore. It is no longer fanned out to member DBs (no mapping table — `external_org_option_mappings` and `deal_hierarchy_links` were dropped 2026-05-27). Instead `external_org_db_sync` mirrors the deal pipeline into the single `🏢 External Orgs` Settings DB (see the sub-sync table below).
...
| `external_org_db_sync` (`src/hierarchy/external_org_db_sync.py`) | `public."ReportingNz_deals"` → the single `🏢 External Orgs` Settings DB (`EXTERNAL_ORGS_DB_ID`) | **No canonical mirror, no member-DB fan-out.** Reads ReportingNz_deals live each tick and maintains **one row per deal** in the Settings DB. Each row carries `Name` (deal name, verbatim), `Stage` (select), and `Deal ID` (the Supabase UUID — the stable identity, so no mapping table). Deals in the tracked stages (`Portfolio` + `DD phase` + `Working on a deal (significant effort)` + `Under analysis (team assigned, moderate effort)`) get a row **created**; existing rows are **updated** in place when name/stage drift. **Rows are never deleted** — a deal that leaves the tracked stages keeps its row with `Stage` refreshed to the current value. Stage colors (Portfolio → orange; dealflow → blue) and the dropdown's display order (Portfolio first) live in the DB schema + a Stage-sorted view, not in code. Stage names are comma-stripped via `_sanitize_option_name` (Notion forbids commas in select options); deal names are stored verbatim. Skips with a **benign warning** (`errors=0`) when `EXTERNAL_ORGS_DB_ID` is unset. Re-uses Supabase HTTP helpers from `canonical_mirror_sync`. |
