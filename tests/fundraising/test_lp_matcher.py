"""Tests for the LP matcher — one LP per meeting or skip."""
...
def test_matcher_returns_single_match_via_email(fake_affinity):
    # Luciano-style: person record has opportunity_ids pointing at an LP entry
...
def test_matcher_ignores_opportunities_outside_funnel(fake_affinity):
    # Person has three opportunity_ids, only one is on the LP Funnel
    fake_affinity.search_persons.return_value = [
...
def test_matcher_skips_on_multiple_matches(fake_affinity):
    # Two attendees from two different LPs in the funnel
...
def test_matcher_domain_fallback(fake_affinity):
    # No per-email match, but a colleague at the same domain is on the funnel
    call_log = []
...
def test_matcher_returns_none_when_no_candidate(fake_affinity):
    fake_affinity.search_persons.return_value = [
...
def test_matcher_reads_email_key_when_present(fake_affinity):
    """Attendees may carry an explicit ``email`` field rather than reusing ``id``."""
    fake_affinity.search_persons.return_value = [
