...
Filter + sort + color rules (per spec PR4):
...
Hierarchy linkage (``public.deal_hierarchy_links``): for each filtered deal,
attempt match-by-sanitized-name against ``hierarchy_rows`` children of the
two Tier 0 / Tier 1 anchors specified by the operator:

  * Portfolio stage → Tier 1 children of ``Value Creation for Portfolio``
  * Dealflow stages → Tier 2 children of ``Dealflow - Main Opportunities``

Match → upsert link row (1:1 deal_id → hierarchy_page_id). No match → log
warning, skip the link (option still gets created/renamed/archived); operator
creates the Hierarchy row later, next tick links automatically.

Sharing: Supabase HTTP helpers via cross-import from ``canonical_mirror_sync``;
sanitization rule from ``macro_block_sync``.

Non-goals:
  * Auto-create Hierarchy DB Tier 1 / Tier 2 rows for new deals.
...
# Stage taxonomy (hard-coded — revisit when the Sales team changes it).
...
# Hierarchy DB anchor pages where the operator parks deal-specific rows.
# Portfolio deals → Tier 1 children of "Value Creation for Portfolio".
# Dealflow deals  → Tier 2 children of "Dealflow - Main Opportunities".
_HIERARCHY_ANCHOR_PORTFOLIO = "c3a645bf-edae-4176-9373-4b0f958f3c72"
_HIERARCHY_ANCHOR_DEALFLOW = "009aebf3-8d24-4862-b67f-0978390db56b"
...
def _load_hierarchy_children(parent_id: str) -> dict[str, str]:
    """Return ``{sanitized_name: notion_page_id}`` for active children of ``parent_id``."""
    parent = urllib.parse.quote(parent_id)
    raw = _http(
        "GET",
        f"/rest/v1/hierarchy_rows?select=notion_page_id,name"
        f"&parent_notion_page_id=eq.{parent}"
        "&deleted_at=is.null&active=eq.true"
        "&limit=10000",
    ) or []
    out: dict[str, str] = {}
    for r in raw:
        ...
        if not page_id or not name:
            continue
        out[_sanitize_option_name(name)] = page_id
    return out
...
# ---------------------------------------------------------------------------
# Hierarchy link maintenance (deal_hierarchy_links)
# ---------------------------------------------------------------------------


def _upsert_hierarchy_links(
    canonical_deals: list[_CanonicalDeal],
    portfolio_targets: dict[str, str],
    dealflow_targets: dict[str, str],
    *,
    report: SyncReport,
) -> None:
    """Match each deal to its Hierarchy DB row (Tier 1 PortCo / Tier 2 Workstream)
    by sanitized name and upsert to ``deal_hierarchy_links``.

    Match misses logged but non-fatal — the External Org option is still
    propagated; the link materialises on a future tick when the operator adds
    the Hierarchy row.
    """
    now_iso = datetime.now(tz=timezone.utc).isoformat()
    ...
    for deal in canonical_deals:
        target_index = (
            portfolio_targets if deal.stage == _STAGE_PORTFOLIO
            else dealflow_targets
        )
        hierarchy_page_id = target_index.get(_sanitize_option_name(deal.name))
        if not hierarchy_page_id:
            unmatched += 1
            report.details.append(
                f"deal {deal.deal_id[:8]} ({deal.name!r}, stage={deal.stage!r}) "
                "→ no matching Hierarchy row (link skipped)",
            )
            continue
        upserts.append({
            "deal_id": deal.deal_id,
    ...
    if upserts:
        try:
            _http(
                "POST",
                "/rest/v1/deal_hierarchy_links?on_conflict=deal_id",
                body=upserts,
                prefer="resolution=merge-duplicates,return=minimal",
            )
            logger.info(
                "external_org_applier_sync: linked %d deal(s) to Hierarchy "
                "rows; %d unmatched",
                len(upserts), unmatched,
            )
        except Exception as e:  # noqa: BLE001
            report.errors += 1
            report.details.append(
                f"deal_hierarchy_links upsert failed: {type(e).__name__}: {e}",
            )
            logger.exception(
                "external_org_applier_sync: deal_hierarchy_links upsert failed",
            )
...
def sync(client: NotionClientWrapper, config: SyncConfig) -> SyncReport:
    ...
    # ----- Hierarchy linkage indices -----
    try:
        portfolio_targets = _load_hierarchy_children(_HIERARCHY_ANCHOR_PORTFOLIO)
        dealflow_targets = _load_hierarchy_children(_HIERARCHY_ANCHOR_DEALFLOW)
    except Exception as e:  # noqa: BLE001
        # Non-fatal — linking is analytical, not on the critical path.
        report.details.append(
            f"hierarchy children fetch failed: {type(e).__name__}: {e} — "
            "links not refreshed this tick",
        )
        logger.warning(
            "external_org_applier_sync: hierarchy children fetch failed: %s", e,
        )
        portfolio_targets, dealflow_targets = {}, {}
    ...
    for member_db in member_dbs:
        ...
        if not plan.changed and not plan.renames:
            logger.debug(
    ...
    # ----- Hierarchy link maintenance (runs once per tick, not per-member) -----
    if not config.dry_run:
        _upsert_hierarchy_links(
            canonical_deals=[
                d for d in canonical_deals if d.deal_id not in skip_deal_ids
            ],
            portfolio_targets=portfolio_targets,
            dealflow_targets=dealflow_targets,
            report=report,
        )
